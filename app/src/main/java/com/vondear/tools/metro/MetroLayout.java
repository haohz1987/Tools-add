...
public class MetroLayout extends RelativeLayout {
    ...
    //sparseArray正序插入时比hashmap效率高，但倒序时效率低
     ...
     * @param column      列数
     * @param cellMargin  间隙
     * @param showMaxLine 适配尺寸，在winMetro中时不再是尺寸了
     */
    public MetroLayout(Context context, int row, int column, int cellMargin, int showMaxLine) {
        this(context);
        this.setTag(0);
        id = 100000;
        if (metros != null)
    ...
    private void init() {
        ...
        if (point.y / realRaw < point.x / column) {
            pieceH = (point.y / realRaw) - dip2px(margin);
            pieceW = (point.y / realRaw) - dip2px(margin);
        } else {
            pieceH = (point.x / column) - dip2px(margin);
            pieceW = (point.x / column) - dip2px(margin);
        }
